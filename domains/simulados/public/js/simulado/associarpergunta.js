...
function renderTable() {
  ...
  if (pageItems.length === 0) {
    ...
    pageItems.forEach(questao => {
      const idStr = String(questao.id_questao);
      const titulo = questao.titulo ?? `Questão ${idStr}`;
      const isChecked = selectedQuestions.includes(idStr) ? 'checked' : '';
        ...
        <td>
          <div class="form-check">
            <input 
              ...
              value="${idStr}" ${isChecked}>
            <label for="questao-${idStr}" class="form-check-label">
              <span class="visually-hidden">Selecionar </span>${titulo}
            </label>
          </div>
        </td>
